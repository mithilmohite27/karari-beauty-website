...
const getCachedSettings = cachedStorefrontRead(readSettingsFromSupabase, ["storefront-site-settings"], [CACHE_TAGS.siteSettings]);

export async function getSiteSettings() {
  // Called from generateMetadata, the root layout and page bodies, so an
  // uncached read costs several identical round trips per request.
  return (await getCachedSettings()) || defaultSiteSettings;
}
...
export async function updateAdminSiteSettings(payload) {
  ...
  // Settings drive the root layout and metadata on every page.
  revalidateStorefront(CACHE_TAGS.siteSettings);
  return {
    data: mergeSettings(data.value),
    mode: "supabase"
  };
