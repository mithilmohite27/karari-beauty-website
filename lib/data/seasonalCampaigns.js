...
async function readActiveSeasonalCampaign(today) {
...
export async function getActiveSeasonalCampaign() {
  // The query filters on today's date, so the date is part of the cache key —
  // otherwise a campaign that ended overnight would keep being served.
  const today = new Date().toISOString().slice(0, 10);

  const cachedRead = cachedStorefrontRead(
    () => readActiveSeasonalCampaign(today),
    ["storefront-active-campaign", today],
      ...
      data: [
        {
          ...
          offerLabel: localSeasonalCampaign.offerLabel || "Festive offers live now",
...
export async function createAdminCampaign(input) {
  ...
  revalidateStorefront(CACHE_TAGS.campaigns);
  return mapCampaign(data);
}
