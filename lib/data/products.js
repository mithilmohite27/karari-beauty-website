import { products as localProducts } from "@/data/products";
import { CACHE_TAGS, cachedStorefrontRead, revalidateStorefront } from "@/lib/cache";
import { generateSlug } from "@/lib/productIdentifiers";
import { filterStorefrontProducts } from "@/lib/storefrontCatalog";
import { createAdminSupabaseClient } from "@/lib/supabase/admin";
import { createServerSupabaseClient } from "@/lib/supabase/server";
import { variantBlur, variantSrc } from "@/lib/imageVariants";

/**
 * Columns the storefront actually renders. Selecting these instead of `*`
 * keeps new admin-only columns from silently widening every catalog read.
 */
const STOREFRONT_PRODUCT_COLUMNS = [
  "id",
  "slug",
  "name",
  "sku",
  "category_id",
  "category_name",
  "category_slug",
  "price",
  "original_price",
  "discount_label",
  "rating",
  "badge",
  "offer",
  "image_url",
  // Pre-generated R2 variant metadata for the primary image. `{}` until the
  // backfill has processed the row - see scripts/migrate-images-to-r2.mjs.
  "image_variants",
  "description",
  "short_description",
  "is_featured",
  "is_active",
  "stock_status",
  "cod_available",
  "tags",
  "sort_order",
  "created_at",
  "updated_at"
].join(", ");

function warnFallback(source, error) {
  console.warn(`[karari-data] ${source} Supabase read failed. Falling back to local data.`, error?.message || error);
}

/**
 * Swap an image reference for its CDN equivalent when one exists.
 *
 * Deliberately applied after gallery normalisation rather than during it: the
 * de-duplication and main-image selection below compare raw image_url values
 * against products.image_url, and rewriting the URLs first would break every
 * one of those comparisons.
 *
 * Returns the original entry untouched when the image has not been migrated,
 * which covers the 12 products still pointing at Unsplash stock photos as well
 * as anything uploaded since the last backfill run.
 */
function applyCdnVariants(image) {
  const src = variantSrc(image.variants);
  if (!src) return image;

  return {
    ...image,
    url: src,
    imageUrl: src,
    // Kept so callers can still reach the Supabase original - the storefront
    // does not need it, but order records and admin screens do.
    originalUrl: image.imageUrl,
    blurDataURL: variantBlur(image.variants) || undefined
  };
}

function normalizeProductGallery(row, productImage) {
  const sourceImages = Array.isArray(row.product_images)
    ? row.product_images
    : Array.isArray(row.galleryImages)
      ? row.galleryImages
      : Array.isArray(row.images)
        ? row.images
        : [];

  const mappedImages = sourceImages
    .map((image, index) => {
      const imageUrl = image.image_url || image.imageUrl || image.url || "";
      if (!imageUrl) return null;

      return {
        id: image.id || `${row.id || row.slug || "product"}-image-${index}`,
        url: imageUrl,
        imageUrl,
        altText: image.alt_text || image.altText || row.name || "Product image",
        isMain: Boolean(image.is_main ?? image.isMain),
        sortOrder: Number(image.sort_order ?? image.sortOrder ?? index),
        createdAt: image.created_at || image.createdAt || "",
        variants: image.variants || image.imageVariants || {}
      };
    })
    .filter(Boolean)
    .sort((a, b) => Number(b.isMain) - Number(a.isMain) || a.sortOrder - b.sortOrder || String(a.createdAt).localeCompare(String(b.createdAt)));

  const gallery = [];
  const seenUrls = new Set();
  const addImage = (image) => {
    const normalizedUrl = String(image?.imageUrl || "").trim();
    if (!normalizedUrl || seenUrls.has(normalizedUrl)) return;
    seenUrls.add(normalizedUrl);
    gallery.push(image);
  };

  const selectedMain = mappedImages.find((image) => image.isMain && image.imageUrl === productImage)
    || mappedImages.find((image) => image.imageUrl === productImage)
    || mappedImages.find((image) => image.isMain);

  if (selectedMain) addImage({ ...selectedMain, isMain: true });
  if (productImage) {
    addImage({
      id: `${row.id || row.slug || "product"}-main`,
      url: productImage,
      imageUrl: productImage,
      altText: row.name || "Product image",
      isMain: true,
      sortOrder: -1,
      createdAt: "",
      variants: row.image_variants || row.imageVariants || {}
    });
  }
  mappedImages.forEach(addImage);

  return gallery.map(applyCdnVariants);
}

function mapProduct(row) {
  const productImage = row.image_url || row.image || "";
  const galleryImages = normalizeProductGallery(row, productImage);
  // galleryImages has already been rewritten to CDN paths where variants exist,
  // so `image` follows automatically and every existing <ProductImage src=...>
  // call site picks up responsive variants with no change.
  const mainImage = galleryImages[0]?.imageUrl || variantSrc(row.image_variants) || productImage;

  return {
    id: row.id || row.slug,
    slug: row.slug,
    name: row.name,
    sku: row.sku || "",
    categoryId: row.category_id || "",
    category: row.category_name || "",
    categorySlug: row.category_slug || "",
    price: Number(row.price || 0),
    originalPrice: row.original_price === null ? undefined : Number(row.original_price || 0),
    discountLabel: row.discount_label || "",
    rating: row.rating === null ? undefined : String(row.rating),
    badge: row.badge || "",
    offer: row.offer || "",
    image: mainImage,
    // The unrewritten Supabase URL. `image` above may be a /cdn/ path, which is
    // a rendering detail - writing one back into products.image_url would
    // corrupt the row, so every admin edit form must read this field instead.
    imageOriginal: productImage,
    imageVariants: row.image_variants || {},
    imageBlur: galleryImages[0]?.blurDataURL || variantBlur(row.image_variants) || "",
    galleryImages,
    description: row.description || "",
    shortDescription: row.short_description || row.description || "",
    isFeatured: Boolean(row.is_featured),
    isActive: row.is_active !== false,
    stockStatus: row.stock_status || "",
    codAvailable: Boolean(row.cod_available),
    tags: row.tags || [],
    sortOrder: row.sort_order || 0,
    createdAt: row.created_at,
    updatedAt: row.updated_at || row.updatedAt || ""
  };
}

const PRODUCT_STOCK_STATUSES = new Set(["in_stock", "low_stock", "out_of_stock", "made_to_order", "preorder"]);
const PRODUCT_MUTATION_FIELDS = new Set([
  "name",
  "slug",
  "sku",
  "category_id",
  "category_slug",
  "category_name",
  "description",
  "short_description",
  "price",
  "original_price",
  "discount_label",
  "rating",
  "badge",
  "offer",
  "image_url",
  // Accepted so an admin upload can save the image and its freshly generated
  // R2 variants in one write. Any image_url change without it clears the
  // variants instead - see normalizeAdminProductInput.
  "image_variants",
  "tags",
  "is_featured",
  "is_active",
  "cod_available",
  "stock_status",
  "sort_order"
]);

export class ProductAdminError extends Error {
  constructor(message, status = 400, details = {}) {
    super(message);
    this.name = "ProductAdminError";
    this.status = status;
    this.details = details;
  }
}

function mapProductImage(row) {
  return {
    id: row.id,
    productId: row.product_id,
    imageUrl: row.image_url,
    storagePath: row.storage_path || "",
    altText: row.alt_text || "",
    isMain: Boolean(row.is_main),
    sortOrder: row.sort_order || 0,
    createdAt: row.created_at,
    // Admin screens render the Supabase original with a native <img>, so
    // imageUrl above stays raw. Variants are surfaced only so the UI can show
    // whether an image has been migrated yet.
    variants: row.variants || {}
  };
}

function emptyToNull(value) {
  if (value === undefined) return undefined;
  if (typeof value === "string" && value.trim() === "") return null;
  return value;
}

function parseOptionalNumber(value, fieldLabel) {
  const normalized = emptyToNull(value);
  if (normalized === undefined || normalized === null) return null;
  const numberValue = Number(normalized);
  if (!Number.isFinite(numberValue) || numberValue < 0) {
    throw new ProductAdminError(`${fieldLabel} must be a valid number greater than or equal to 0.`);
  }
  return numberValue;
}

function parseTags(value) {
  if (Array.isArray(value)) return value.map((tag) => String(tag).trim()).filter(Boolean);
  if (typeof value === "string") return value.split(",").map((tag) => tag.trim()).filter(Boolean);
  return [];
}

function isUuid(value) {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(value || ""));
}

function createMutationError(action, error) {
  const message = error?.message || "";
  const normalizedMessage = message.toLowerCase();

  if (error?.code === "23505" || normalizedMessage.includes("duplicate key")) {
    if (normalizedMessage.includes("sku")) return new ProductAdminError("A product with this SKU already exists.", 409);
    return new ProductAdminError("A product with this slug already exists.", 409);
  }

  console.error(`[admin-products] ${action} failed`, {
    code: error?.code,
    message: error?.message,
    details: error?.details,
    hint: error?.hint
  });

  return new ProductAdminError(`Unable to ${action}. ${message || "Please check the product details and try again."}`, 500);
}

function isMissingColumnError(error, columnName) {
  const message = String(error?.message || "").toLowerCase();
  return (
    error?.code === "PGRST204" &&
    message.includes(columnName.toLowerCase()) &&
    message.includes("schema cache")
  );
}

function omitPayloadFields(payload, fields) {
  const nextPayload = { ...payload };
  for (const field of fields) delete nextPayload[field];
  return nextPayload;
}

function normalizeAdminProductInput(input = {}, { partial = false } = {}) {
  const payload = {};

  for (const [key, value] of Object.entries(input)) {
    if (PRODUCT_MUTATION_FIELDS.has(key)) payload[key] = emptyToNull(value);
  }

  if (!partial || "name" in payload) {
    if (!payload.name) throw new ProductAdminError("Product name is required.");
    payload.name = String(payload.name).trim();
  }

  if (!partial || "slug" in payload) {
    const slug = generateSlug(payload.slug || payload.name);
    if (!slug) throw new ProductAdminError("Product slug is required.");
    payload.slug = slug;
  }

  if (!partial || "price" in payload) {
    if (payload.price === undefined || payload.price === null) throw new ProductAdminError("Product price is required.");
    payload.price = parseOptionalNumber(payload.price, "Product price");
    if (payload.price <= 0) throw new ProductAdminError("Product price must be greater than zero.");
  }

  if ("original_price" in payload) payload.original_price = parseOptionalNumber(payload.original_price, "Original price");
  if ("rating" in payload) {
    const rating = parseOptionalNumber(payload.rating, "Rating");
    if (rating !== null && rating > 5) throw new ProductAdminError("Rating must be between 0 and 5.");
    payload.rating = rating;
  }

  if ("sort_order" in payload) {
    const sortOrder = payload.sort_order === null ? 0 : Number(payload.sort_order);
    if (!Number.isFinite(sortOrder)) throw new ProductAdminError("Sort order must be a valid number.");
    payload.sort_order = Math.trunc(sortOrder);
  }

  if ("tags" in payload) payload.tags = parseTags(payload.tags);

  if ("stock_status" in payload || !partial) {
    payload.stock_status = payload.stock_status || "in_stock";
    if (!PRODUCT_STOCK_STATUSES.has(payload.stock_status)) {
      throw new ProductAdminError("Stock status is not valid.");
    }
  }

  if ("is_featured" in payload) payload.is_featured = Boolean(payload.is_featured);
  if ("is_active" in payload) payload.is_active = payload.is_active !== false;
  if ("cod_available" in payload) payload.cod_available = Boolean(payload.cod_available);

  for (const field of ["sku", "category_id", "category_slug", "category_name", "description", "short_description", "discount_label", "badge", "offer", "image_url"]) {
    if (field in payload && typeof payload[field] === "string") payload[field] = payload[field].trim() || null;
  }

  if ("category_id" in payload && payload.category_id && !isUuid(payload.category_id)) payload.category_id = null;
  if (payload.sku) payload.sku = String(payload.sku).toUpperCase();

  // Any write that changes image_url invalidates the stored variants: they
  // describe the previous picture. Clearing them drops the product back to
  // serving its original URL - a slower image, but the correct one - and marks
  // the row for the next backfill run via the idx_products_unmigrated index.
  if ("image_url" in payload && !("image_variants" in payload)) payload.image_variants = {};

  return payload;
}

async function assertCategoryIsValid(adminClient, payload) {
  if (!payload.category_id && !payload.category_slug) return payload;

  let query = adminClient.from("categories").select("id, name, slug").limit(1);
  if (payload.category_id) query = query.eq("id", payload.category_id);
  else query = query.eq("slug", payload.category_slug);

  const { data, error } = await query.maybeSingle();

  if (error) throw new ProductAdminError("Unable to validate the selected category.", 500);
  if (!data) throw new ProductAdminError("Selected category is not valid.");

  return {
    ...payload,
    category_id: data.id,
    category_slug: data.slug,
    category_name: payload.category_name || data.name
  };
}

async function assertSlugIsUnique(adminClient, slug, currentId) {
  if (!slug) return;

  const { data, error } = await adminClient.from("products").select("id").eq("slug", slug).maybeSingle();
  if (error) throw new ProductAdminError("Unable to validate product slug.", 500);
  if (data && data.id !== currentId) throw new ProductAdminError("A product with this slug already exists.", 409);
}

async function assertSkuIsUnique(adminClient, sku, currentId) {
  if (!sku) return;

  const { data, error } = await adminClient.from("products").select("id").eq("sku", sku).maybeSingle();
  if (error) throw new ProductAdminError("Unable to validate product SKU.", 500);
  if (data && data.id !== currentId) throw new ProductAdminError("A product with this SKU already exists.", 409);
}

function getAdminClientOrThrow() {
  const adminClient = createAdminSupabaseClient();
  if (!adminClient) throw new ProductAdminError("Supabase setup required", 503);
  return adminClient;
}

function normalizeProductIds(productIds) {
  const ids = Array.isArray(productIds) ? productIds.map((id) => String(id || "").trim()).filter(Boolean) : [];
  return [...new Set(ids)];
}

function assertOwnerCanDelete(adminUser) {
  if (adminUser?.role !== "owner") {
    throw new ProductAdminError("Only owner can permanently delete products.", 403);
  }
}

async function getOrderLinkedProductIds(adminClient, productIds) {
  if (!productIds.length) return [];

  const { data, error } = await adminClient
    .from("order_items")
    .select("product_id")
    .in("product_id", productIds);

  if (error) throw new ProductAdminError("Unable to check product order history.", 500);
  return [...new Set((data || []).map((item) => item.product_id).filter(Boolean))];
}

/**
 * Trim a product for list use.
 *
 * Every product in the catalogue is serialised into the page so client-side
 * search can work, and it is serialised twice - once in the rendered markup and
 * again in the RSC payload. Fields no list view reads are therefore paid for
 * twice per product, per page.
 *
 * galleryImages is the expensive one: list reads do not join product_images, so
 * each entry is just the main image wrapped in seven fields, and it measured
 * ~70 KB across the catalogue. The product detail page fetches its own gallery
 * via getProductBySlug, so nothing that needs it loses it.
 */
function toListProduct(product) {
  const { galleryImages, ...listProduct } = product;
  return listProduct;
}

async function readSupabaseProducts() {
  const supabase = createServerSupabaseClient();
  if (!supabase) return null;

  const { data, error } = await supabase
    .from("products")
    .select(STOREFRONT_PRODUCT_COLUMNS)
    .eq("is_active", true)
    .order("sort_order", { ascending: true })
    .order("name", { ascending: true });

  if (error) {
    warnFallback("products", error);
    return null;
  }

  return data.map((row) => toListProduct(mapProduct(row)));
}

const getCachedProducts = cachedStorefrontRead(readSupabaseProducts, ["storefront-products"], [CACHE_TAGS.products]);

export async function getProducts() {
  // Hiding a category from navigation does not hide its products: the homepage
  // grid lists every active product when no filter is applied. Both must be
  // filtered or retired stock stays purchasable.
  return filterStorefrontProducts((await getCachedProducts()) || localProducts);
}

export async function getFeaturedProducts() {
  const products = await getProducts();
  return products.filter((product) => product.isFeatured);
}

export async function getProductsByCategorySlug(categorySlug) {
  const products = await getProducts();
  return products.filter((product) => product.categorySlug === categorySlug);
}

async function readSupabaseProductBySlug(slug) {
  const supabase = createServerSupabaseClient();
  if (!supabase) return { ok: false, product: null };

  const { data, error } = await supabase
    .from("products")
    .select(`${STOREFRONT_PRODUCT_COLUMNS}, product_images(id, image_url, storage_path, alt_text, is_main, sort_order, created_at, variants)`)
    .eq("slug", slug)
    .eq("is_active", true)
    .maybeSingle();

  if (error) {
    warnFallback("product detail", error);
    return { ok: false, product: null };
  }

  return { ok: true, product: data ? mapProduct(data) : null };
}

export async function getProductBySlug(slug) {
  const cachedRead = cachedStorefrontRead(
    () => readSupabaseProductBySlug(slug),
    ["storefront-product", String(slug)],
    [CACHE_TAGS.products]
  );

  const { ok, product } = await cachedRead();

  // Only fall back to bundled data when Supabase itself was unreachable. A
  // successful query that returned nothing means the product is genuinely gone,
  // and must 404 rather than resurrect a stale local copy.
  if (ok) return product;

  return localProducts.find((entry) => entry.slug === slug) || null;
}

export async function getRelatedProducts(product, limit = 4) {
  if (!product) return [];

  const products = await getProducts();
  const sameCategory = products.filter((item) => item.slug !== product.slug && item.categorySlug === product.categorySlug);
  const featuredFill = products.filter((item) => item.slug !== product.slug && item.categorySlug !== product.categorySlug && item.isFeatured);

  return [...sameCategory, ...featuredFill].slice(0, limit);
}

export async function getAdminProducts() {
  const adminClient = createAdminSupabaseClient();

  if (!adminClient) {
    return {
      data: localProducts.map((product, index) => ({
        ...product,
        isActive: product.isActive !== false,
        stockStatus: product.stockStatus || "in_stock",
        sortOrder: product.sortOrder || index
      })),
      mode: "fallback"
    };
  }

  const { data, error } = await adminClient.from("products").select("*").order("created_at", { ascending: false });

  if (error) {
    warnFallback("admin products", error);
    return {
      data: localProducts,
      mode: "fallback"
    };
  }

  return {
    data: data.map(mapProduct),
    mode: "supabase"
  };
}

export async function getAdminProductById(id) {
  const adminClient = getAdminClientOrThrow();

  const { data, error } = await adminClient.from("products").select("*").eq("id", id).maybeSingle();
  if (error) throw new ProductAdminError("Unable to load product.", 500);
  if (!data) throw new ProductAdminError("Product not found.", 404);

  return mapProduct(data);
}

export async function createAdminProduct(input) {
  const adminClient = getAdminClientOrThrow();
  let payload = normalizeAdminProductInput(input);
  payload = await assertCategoryIsValid(adminClient, payload);
  await assertSlugIsUnique(adminClient, payload.slug);
  await assertSkuIsUnique(adminClient, payload.sku);

  let { data, error } = await adminClient.from("products").insert(payload).select("*").single();

  if (isMissingColumnError(error, "cod_available")) {
    console.warn("[admin-products] products.cod_available is missing in Supabase schema cache. Retrying create without optional COD field.");
    const retryResult = await adminClient.from("products").insert(omitPayloadFields(payload, ["cod_available"])).select("*").single();
    data = retryResult.data;
    error = retryResult.error;
  }

  if (error) throw createMutationError("create product", error);

  revalidateStorefront(CACHE_TAGS.products);
  return mapProduct(data);
}

export async function updateAdminProduct(id, input) {
  const adminClient = getAdminClientOrThrow();
  let payload = normalizeAdminProductInput(input, { partial: true });
  payload = await assertCategoryIsValid(adminClient, payload);
  if (payload.slug) await assertSlugIsUnique(adminClient, payload.slug, id);
  if (payload.sku) await assertSkuIsUnique(adminClient, payload.sku, id);

  let { data, error } = await adminClient.from("products").update(payload).eq("id", id).select("*").maybeSingle();

  if (isMissingColumnError(error, "cod_available")) {
    console.warn("[admin-products] products.cod_available is missing in Supabase schema cache. Retrying update without optional COD field.");
    const retryResult = await adminClient.from("products").update(omitPayloadFields(payload, ["cod_available"])).eq("id", id).select("*").maybeSingle();
    data = retryResult.data;
    error = retryResult.error;
  }

  if (error) throw createMutationError("update product", error);

  if (!data) throw new ProductAdminError("Product not found.", 404);
  revalidateStorefront(CACHE_TAGS.products);
  return mapProduct(data);
}

export async function deactivateAdminProduct(id) {
  const adminClient = getAdminClientOrThrow();

  const { data, error } = await adminClient
    .from("products")
    .update({
      is_active: false,
      stock_status: "out_of_stock"
    })
    .eq("id", id)
    .select("id")
    .maybeSingle();

  if (error) throw new ProductAdminError("Unable to deactivate product.", 500);
  if (!data) throw new ProductAdminError("Product not found.", 404);

  revalidateStorefront(CACHE_TAGS.products);
  return {
    ok: true,
    message: "Product deactivated"
  };
}

export async function hardDeleteAdminProduct(id, adminUser) {
  const adminClient = getAdminClientOrThrow();
  assertOwnerCanDelete(adminUser);

  const [blockedId] = await getOrderLinkedProductIds(adminClient, [id]);
  if (blockedId) {
    throw new ProductAdminError("This product is linked to an order. Hide it from website instead.", 409, {
      blocked: [blockedId]
    });
  }

  const { data, error } = await adminClient.from("products").delete().eq("id", id).select("id").maybeSingle();

  if (error) throw new ProductAdminError("Unable to permanently delete product.", 500);
  if (!data) throw new ProductAdminError("Product not found.", 404);

  revalidateStorefront(CACHE_TAGS.products);
  return {
    ok: true,
    affected: 1,
    blocked: [],
    message: "Product permanently deleted."
  };
}

export async function bulkDeactivateAdminProducts(productIds) {
  const adminClient = getAdminClientOrThrow();
  const ids = normalizeProductIds(productIds);

  if (!ids.length) throw new ProductAdminError("Select at least one product.");
  if (ids.length > 100) throw new ProductAdminError("You can manage up to 100 products at a time.");

  const { data, error } = await adminClient
    .from("products")
    .update({
      is_active: false,
      stock_status: "out_of_stock"
    })
    .in("id", ids)
    .select("id");

  if (error) throw new ProductAdminError("Unable to hide selected products.", 500);

  revalidateStorefront(CACHE_TAGS.products);
  return {
    ok: true,
    affected: data?.length || 0,
    blocked: [],
    message: `${data?.length || 0} selected ${data?.length === 1 ? "product" : "products"} hidden from website.`
  };
}

export async function bulkHardDeleteAdminProducts(productIds, adminUser) {
  const adminClient = getAdminClientOrThrow();
  assertOwnerCanDelete(adminUser);

  const ids = normalizeProductIds(productIds);
  if (!ids.length) throw new ProductAdminError("Select at least one product.");
  if (ids.length > 100) throw new ProductAdminError("You can delete up to 100 products at a time.");

  const blocked = await getOrderLinkedProductIds(adminClient, ids);
  if (blocked.length) {
    throw new ProductAdminError("Some products are linked to orders and cannot be permanently deleted. Hide them from website instead.", 409, {
      blocked,
      blockedCount: blocked.length
    });
  }

  const { data, error } = await adminClient.from("products").delete().in("id", ids).select("id");
  if (error) throw new ProductAdminError("Unable to permanently delete selected products.", 500);

  revalidateStorefront(CACHE_TAGS.products);
  return {
    ok: true,
    affected: data?.length || 0,
    blocked: [],
    message: `${data?.length || 0} selected ${data?.length === 1 ? "product" : "products"} permanently deleted.`
  };
}

export async function getProductImages(productId) {
  const adminClient = getAdminClientOrThrow();

  const { data, error } = await adminClient
    .from("product_images")
    .select("*")
    .eq("product_id", productId)
    .order("sort_order", { ascending: true })
    .order("created_at", { ascending: true });

  if (error) throw new ProductAdminError("Unable to load product images.", 500);
  return data.map(mapProductImage);
}

/**
 * Promote one image to be the product's primary image.
 *
 * products.image_variants must be kept in step with products.image_url or the
 * storefront renders the new main image at the old one's variants - a subtly
 * wrong picture, which is worse than a missing one. When the caller does not
 * already hold the variant record (the common case: an admin clicking "set as
 * main" on an existing gallery image) it is read back from product_images.
 */
export async function setMainProductImage(productId, imageUrl, variants) {
  const adminClient = getAdminClientOrThrow();

  let nextVariants = variants && typeof variants === "object" ? variants : null;

  if (!nextVariants && imageUrl) {
    const { data: sourceImage } = await adminClient
      .from("product_images")
      .select("variants")
      .eq("product_id", productId)
      .eq("image_url", imageUrl)
      .maybeSingle();
    nextVariants = sourceImage?.variants || {};
  }

  const { data, error } = await adminClient
    .from("products")
    .update({
      image_url: imageUrl || null,
      image_variants: imageUrl ? (nextVariants || {}) : {}
    })
    .eq("id", productId)
    .select("*")
    .maybeSingle();

  if (error) throw new ProductAdminError("Unable to update main product image.", 500);
  if (!data) throw new ProductAdminError("Product not found.", 404);

  if (imageUrl) {
    await adminClient.from("product_images").update({ is_main: false }).eq("product_id", productId);
    await adminClient.from("product_images").update({ is_main: true }).eq("product_id", productId).eq("image_url", imageUrl);
  }

  revalidateStorefront(CACHE_TAGS.products);
  return mapProduct(data);
}

export async function addProductImage(productId, input = {}) {
  const adminClient = getAdminClientOrThrow();
  if (!input.image_url) throw new ProductAdminError("Image URL is required.");

  const payload = {
    product_id: productId,
    image_url: String(input.image_url).trim(),
    storage_path: input.storage_path ? String(input.storage_path).trim() : null,
    alt_text: input.alt_text ? String(input.alt_text).trim() : null,
    is_main: Boolean(input.is_main || input.set_as_main),
    sort_order: Number.isFinite(Number(input.sort_order)) ? Math.trunc(Number(input.sort_order)) : 0,
    // Supplied by the media upload route when R2 processing succeeded. Absent
    // for images added by pasting a URL, which the backfill can pick up later.
    variants: input.variants && typeof input.variants === "object" ? input.variants : {}
  };

  const { data, error } = await adminClient.from("product_images").insert(payload).select("*").single();
  if (error) throw new ProductAdminError("Unable to add product image.", 500);

  if (payload.is_main || input.set_as_main) {
    await setMainProductImage(productId, payload.image_url, payload.variants);
  }

  revalidateStorefront(CACHE_TAGS.products);
  return mapProductImage(data);
}

export async function updateProductImage(productId, imageId, input = {}) {
  const adminClient = getAdminClientOrThrow();
  const payload = {};

  if ("alt_text" in input) payload.alt_text = input.alt_text ? String(input.alt_text).trim() : null;
  if ("sort_order" in input) {
    const sortOrder = Number(input.sort_order);
    if (!Number.isFinite(sortOrder)) throw new ProductAdminError("Sort order must be valid.");
    payload.sort_order = Math.trunc(sortOrder);
  }
  if ("is_main" in input || "set_as_main" in input) payload.is_main = Boolean(input.is_main || input.set_as_main);

  const { data, error } = await adminClient
    .from("product_images")
    .update(payload)
    .eq("id", imageId)
    .eq("product_id", productId)
    .select("*")
    .maybeSingle();

  if (error) throw new ProductAdminError("Unable to update product image.", 500);
  if (!data) throw new ProductAdminError("Product image not found.", 404);

  if (payload.is_main) {
    await setMainProductImage(productId, data.image_url);
  }

  revalidateStorefront(CACHE_TAGS.products);
  return mapProductImage(data);
}

function storagePathForRemoval(storagePath) {
  if (!storagePath) return "";
  return storagePath.replace(/^product-images\//, "");
}

export async function deleteProductImage(productId, imageId) {
  const adminClient = getAdminClientOrThrow();

  const { data: image, error: loadError } = await adminClient
    .from("product_images")
    .select("*")
    .eq("id", imageId)
    .eq("product_id", productId)
    .maybeSingle();

  if (loadError) throw new ProductAdminError("Unable to load product image.", 500);
  if (!image) throw new ProductAdminError("Product image not found.", 404);

  const { error } = await adminClient.from("product_images").delete().eq("id", imageId).eq("product_id", productId);
  if (error) throw new ProductAdminError("Unable to remove product image.", 500);

  const removablePath = storagePathForRemoval(image.storage_path);
  if (removablePath) {
    const { error: storageError } = await adminClient.storage.from("product-images").remove([removablePath]);
    if (storageError) console.warn("[admin-product-images] Storage remove failed", storageError.message);
  }

  const { data: product } = await adminClient.from("products").select("image_url").eq("id", productId).maybeSingle();
  if (product?.image_url === image.image_url) {
    const { data: remaining } = await adminClient
      .from("product_images")
      .select("*")
      .eq("product_id", productId)
      .order("sort_order", { ascending: true })
      .order("created_at", { ascending: true })
      .limit(1)
      .maybeSingle();

    await setMainProductImage(productId, remaining?.image_url || null);
  }

  revalidateStorefront(CACHE_TAGS.products);
  return {
    ok: true,
    message: "Product image removed"
  };
}
