...
import { filterStorefrontCategories } from "@/lib/storefrontCatalog";
...
const getCachedCategories = cachedStorefrontRead(getSupabaseCategories, ["storefront-categories"], [CACHE_TAGS.categories]);

export async function getCategories() {
  // The whitelist is applied to both branches on purpose. The fallback is the
  // branch that historically leaked retired categories onto the live site, but
  // filtering only that branch would leave the database free to do the same.
  return filterStorefrontCategories((await getCachedCategories()) || localCategories);
}
...
/**
 * Build the PostgREST `or()` filter matching products in a category.
 *
 * `or()` takes a comma-separated filter string, so any comma, parenthesis or
 * dot in an interpolated value is parsed as filter syntax rather than data.
 * The id arrives from a route parameter, so it is checked against the UUID
 * shape and dropped if it does not match; the slug is restricted to the
 * characters generateSlug can actually produce.
 */
function buildCategoryMatchFilter(category) {
  const clauses = [];
  const id = String(category?.id || "");
  const slug = String(category?.slug || "");

  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)) {
    clauses.push(`category_id.eq.${id}`);
  }
  if (/^[a-z0-9-]+$/i.test(slug)) {
    clauses.push(`category_slug.eq.${slug}`);
  }

  // Never return an empty filter - that would match every product and report a
  // category as in use when it is not.
  if (!clauses.length) throw new CategoryAdminError("Category identifier is not valid.", 400);
...
export async function deactivateAdminCategory(id) {
  ...
  const { count, error: countError } = await adminClient
    ...
    .or(buildCategoryMatchFilter(category))
    .eq("is_active", true);
  ...
  revalidateStorefront(CACHE_TAGS.categories, CACHE_TAGS.products);
  return {
    ok: true,
    message: count
