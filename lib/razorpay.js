...
export class RazorpayAuthError extends Error {
  constructor(message = "Razorpay rejected the configured API credentials.") {
    super(message);
...
/**
 * Trimming is not cosmetic. Pasting a key into a Vercel environment variable
 * very often carries a trailing newline or space; the value then looks correct
 * in the dashboard but is sent as part of the Basic auth credential and
 * Razorpay answers 401 "Authentication failed" - which surfaces at checkout as
 * a generic "Unable to create Razorpay order".
 */
...
export function getRazorpayConfig() {
  return {
    publicKeyId: readKey("NEXT_PUBLIC_RAZORPAY_KEY_ID"),
    keyId: readKey("RAZORPAY_KEY_ID"),
    keySecret: readKey("RAZORPAY_KEY_SECRET"),
    webhookSecret: readKey("RAZORPAY_WEBHOOK_SECRET")
  };
}
...
export async function createRazorpayOrder({ amount, currency = "INR", receipt, notes = {} }) {
  ...
  if (!response.ok) {
    // 401 means the key id and secret are not a valid pair - typically the
    // secret was regenerated in the Razorpay dashboard without updating the
    // environment, or the two come from different key pairs. It is a
    // configuration fault, not a transient API failure, so it is raised
    // distinctly rather than as a generic "try again".
    if (response.status === 401) {
      throw new RazorpayAuthError(
        `Razorpay rejected the API credentials (${data?.error?.description || "Authentication failed"}). ` +
        "Check that RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET are from the same key pair and are current."
      );
    }
