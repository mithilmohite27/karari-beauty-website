...
export default function ProductCard({ product, onView, onAddToCart, onBuyNow, onToggleWishlist, wished }) {
  ...
  const originalPrice = orderDetails.hasValidPrice ? product.originalPrice : null;
  const discountLabel = orderDetails.hasValidPrice ? product.discountLabel || product.offer : "";
    ...
    >
      ...
      <div className="p-3 sm:p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          {!orderDetails.hasValidPrice || (product.stockStatus && product.stockStatus !== "in_stock") ? (
            <span className="inline-flex items-center rounded-full bg-[#FFF8EE] px-2.5 py-1 text-xs font-bold text-[#7A183D]">
              {orderDetails.label}
            </span>
          ) : null}
        </div>
        ...
        <div className="mt-2.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 sm:mt-3">
          <p className="text-base font-bold text-rose sm:text-lg">{orderDetails.hasValidPrice ? format(product.price) : "Price on request"}</p>
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 border-t border-[rgba(122,24,61,0.1)] p-3 sm:p-5">
        <button
          ...
          className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-md border border-[#7A183D] bg-white/88 px-2.5 py-2 text-xs font-bold text-[#7A183D] transition hover:border-[#C9962D] hover:text-[#C9962D] disabled:cursor-not-allowed disabled:border-[#3A2417]/20 disabled:text-[#3A2417]/35 sm:gap-2 sm:px-3 sm:py-2.5 sm:text-sm"
        >
          <ShoppingCart className="h-4 w-4" />
          <span className="hidden min-[370px]:inline">{orderDetails.purchasable ? t("product.addToCart", "Add to Cart") : "Unavailable"}</span>
          <span className="min-[370px]:hidden">{orderDetails.purchasable ? t("product.add", "Add") : "Unavailable"}</span>
        </button>
        ...
        {onBuyNow ? (
          <button
            ...
            disabled={!orderDetails.purchasable}
            className="col-span-2 inline-flex min-h-11 items-center justify-center rounded-md bg-[#7A183D] px-3 py-2 text-xs font-bold text-white transition hover:bg-[#3A2417] disabled:cursor-not-allowed disabled:bg-[#3A2417]/35 sm:text-sm"
          >
            {orderDetails.purchasable ? t("product.buyNow", "Buy Now") : "Unavailable"}
          </button>
        ) : null}
      </div>
    </motion.article>
